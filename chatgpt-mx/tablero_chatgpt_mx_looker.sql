-- =====================================================================
-- Tablero ChatGPT Ads MX - version para Looker Studio (custom query)
-- Sin DECLARE: Looker Studio no admite scripting multi-sentencia.
-- La fecha de corte vive en el CTE param de abajo.
...
--   c) fecha de corte en el CTE param, para excluir el lead de prueba
-- =====================================================================


WITH
-- Cambiar esta fecha al dia que prendas la campana para dejar fuera
-- el lead de prueba del 8 de septiembre.
param AS (SELECT DATE '2026-09-08' AS f),
-- --- INVERSION ---------------------------------------------------------
inv_conector AS (
...
leads AS (
  ...
  WHERE LOWER(COALESCE(g.campana_mercadeo,'')) LIKE '%hatgpt%'
    AND DATE(g.fecha_creacion) >= (SELECT f FROM param)
  GROUP BY 1, 2, 3
)
...
WHERE COALESCE(i.dia, l.dia) >= (SELECT f FROM param)
ORDER BY 1 DESC;
